...
export class PracticeService {
  ...
  async create(createPracticeDto: CreatePracticeDto, user:any) {
    let response = new CreatePracticeResponse();
    try {
        let exam_master_id = createPracticeDto.exam_master_id;
        ...
        if(!exams.length){
            const creation_result_practice_1 = await this.connection.query(
                'call create_practice_1($1, $2);',[exam_master_id,user_id]
                );
            const creation_result_practice_2 = await this.connection.query(
                'call create_practice_2($1, $2);',[exam_master_id,user_id]
                );
            const creation_result_practice_3 = await this.connection.query(
                'call create_practice_3($1, $2);',[exam_master_id,user_id]
                );
            const creation_result_practice_4 = await this.connection.query(
                'call create_practice_4($1, $2);',[exam_master_id,user_id]
                );
